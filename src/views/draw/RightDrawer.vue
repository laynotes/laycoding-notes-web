  ...
  <div class="drawer_container">
      ...
      <div class="drawer_wrap">
        <Form label-position="left" :label-width="85">
          ...
          <div v-show="showGrid">
            ...
            <FormItem label="网格大小">
              <Slider v-model="grid.size" :min="0" :max="30" @on-change="changeGrid1"></Slider>
            </FormItem>
            <FormItem label="网格颜色">
              <ColorPicker v-model="grid.args.color" @on-change="changeGrid1"/>
            </FormItem>
            <FormItem label="网格线宽度">
              <Slider v-model="grid.args.thickness" :min="0" :max="20" @on-change="changeGrid1"></Slider>
            </FormItem>
          </div>
    ...
    <div v-if="drawerType === 'node'">
      ...
      <div class="drawer_wrap">
        <Form label-position="left" :label-width="80">
          <FormItem label="节点文本">
            <Input v-model="drawerNode.nodeText" @on-change="changeNodeText" />
          </FormItem>
          <FormItem label="层级">
            <Input v-model="drawerNode.zIndex" @on-change="changeZIndex"/>
          </FormItem>
    ...
    <div v-if="drawerType === 'edge'">
      ...
      <div class="drawer_wrap">
        <Form label-position="left" :label-width="80">
          <FormItem label="线条文本">
            <Input v-model="drawerEdge.EdgeText" @on-change="changeEdgeText" />
          </FormItem>
          <FormItem label="线条宽度">
...
<script>
import {getFontSize} from './draw.js';
...
export default {
  ...
  watch: {
    selectCell: {
      handler(val) {
        if (val) {
          if (val.isNode()) { //节点
            this.drawerNode.zIndex = val.store.data.zIndex;
  ...
  methods: {
    ...
    changeNodeText() {
      const str = this.selectCell.store.data.attrs.label.text;

      console.log(str)

      if (this.drawerNode.nodeText === '') {
        this.drawerNode.nodeText = str;
        return;
      }
      ...
      const arr = getFontSize(this.drawerNode.nodeText, fontSize);
      console.log(arr);
      //const t_width = arr[0] > 40 ? arr[0] + 20 : 40;
      const oldW = this.selectCell.size().width;
      const diff_width = oldW - getFontSize(str, fontSize)[0];
      console.log(this.selectCell.size().width)
      //this.selectCell.store.width = arr[0] + diff_width;.position(30, 30)

      const pos = this.selectCell.position();

      this.selectCell.resize(arr[0] + diff_width, this.selectCell.size().height);
      this.selectCell.position(pos.x - (arr[0] + diff_width - oldW) / 2, pos.y, {deep: true})
    },
    changeStroke(val) {
    ...
    changeZIndex() {
      this.selectCell.setZIndex(parseInt(this.drawerNode.zIndex))
    },
    // 删除
    deleteNode() {
