...
<template>
  <div class="le-editor-toolbar">
    ...
    <ul class="le-editor-tools le-not-select">
      ...
      <li v-if="toolbar.skin" class="skin">
        <a href="javascript:;" title="代码主题" unselectable="on">
          <i class="iconfont icon-meihuadaima" name="skin" unselectable="on" @click.stop="selectCode()"></i>
        </a>
        <div class="dropdown-content" v-if="codeFlag">
          <a
              v-for="(item,index) in codeThemes"
              :class="{'current-skin':item === currentTheme}"
              v-bind:key="index"
              v-text="item"
              @click="selectCodeTheme(item)"
          ></a>
        </div>
      </li>
      ...
      <li>
        <a href="javascript:;" title="预览" unselectable="on">
          <i class="fa fa-eye" name="预览" unselectable="on" @click.stop="review()"></i>
        </a>
      </li>
...
<script>
import { keydownListener } from "le-markdown-editor/src/lib/core/keydown-listener";
...
export default {
  ...
  data() {
    return {
      codeThemes:["github","github-dark","default","monokai-sublime",'atom-one-dark','atom-one-light'],
      modalStyle: {},
      insertImgFlag: false,
  ...
  mounted() {
    ...
    this.currentTheme = localStorage.getItem("code_theme") ? localStorage.getItem("code_theme") : "github-dark";
  },
};
</script>
