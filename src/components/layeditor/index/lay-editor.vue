    ...
    <div class="le-editor-container">
      ...
      <transition name="le-editor-right-animation">
        <div class="le-editor-right"
          ...
          <lay-preview :style="{fontSize:font.preview + 'px'}" :hljsCss="hljsCss" :value="html" :html="htmlValue"
                       ref="markdownBody"></lay-preview>
...
import {insertImg, insertTable, simpleClick} from 'le-markdown-editor/src/lib/core/toolbar_click'
import md from 'le-markdown-editor/src/lib/core/markdown'
import config from 'le-markdown-editor/src/lib/config'
import {uploadToGithub, uploadToServer} from 'le-markdown-editor/src/lib/utils/upload'
// 自定义组件
import toolbar from '../toolbar/lay-toolbar'
import LayPreview from "../preview/lay-preview"
import LeContextMenu from "le-markdown-editor/src/components/menu/le-context-menu"
...
export default {
  ...
  watch: {
    ...
    value(value) {
      if (this.origin !== value) {
        ...
        this.hljsCss = localStorage.getItem("code_theme");
      }
    },
  ...
  methods: {
      ...
      this.editor.on('scroll', (instance) => {
        ...
        timer = setTimeout(() => {
          ...
          if (editorScrollInfo.clientHeight + editorScrollInfo.top === editorScrollInfo.height) {
            ...
            mdBody.scrollTop = editorScrollInfo.top * (mdBody.clientHeight / editorScrollInfo.clientHeight) - 100;
          }
        }, 300)
      })
      // 拖拽事件
    ...
    setCodeTheme(theme) {
      ...
      this.hljsCss = localStorage.getItem("code_theme") ? localStorage.getItem("code_theme") : "github";
...
// 上传
    upload(file) {
      ...
      if (this.config.imageUploader.custom) {
        ...
        if (this.config.imageUploader.type === 'server') {
          ...
          const urlPrefix = window.location.origin + "/api";
          ...
          service({
            ...
            if (res.data) {
              this.insertImg(urlPrefix + res.data.fileUrl, "image")
            }
    ...
    ,
    setMdValue() {
      const mdv = this.editor.getValue()

      this.origin = mdv;

      if (mdv.length > 20000 && !this.isFirst) {
        return;
      }
      this.isFirst = false;
      const config_opt = {};
      config_opt.themeType = "github";
      config_opt.currentFont = null;
      config_opt.currentSize = null;
      config_opt.citeStatus = null;
      // eslint-disable-next-line no-unreachable
      const option = sessionStorage.getItem("file_config") ? JSON.parse(sessionStorage.getItem("file_config")) : config_opt;
